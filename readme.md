...
顺便也可以提醒入门者避开 Windows 下的几个坑，少遭点罪。
...
比如我在 Surface 平板上写字，Vim 结合键盘和屏幕键盘写字的感觉最好，也算是在新时代的新生吧。
...
输入一些字符之后，如果你点击工具栏上的 `保存` 按钮，Vim 会提示说：
E32: 没有文件名
这是怎么回事呢？一般的编辑器不是在保存未命名文件时会弹出系统浏览器，让你选择保存的路径和文件名吗？

如果一开始没有指定文件名，需要使用 `save` 指令指定路径和文件名。
